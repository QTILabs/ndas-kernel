...
enum pcapng_opt_shb { PCAPNG_OPT_SHB_HARDWARE = 2, PCAPNG_OPT_SHB_OS, PCAPNG_OPT_SHB_USERAPPL };
 ...
 * pcapng_add_option()
 *****************************************************************************/
static struct pcapng_option* pcapng_add_option(struct pcapng_option* opt, uint16_t type, uint16_t length,
                                               const void* data) {
    if (opt == NULL)
        return NULL;
    ...
    return (struct pcapng_option*)((uint8_t*)opt + pcapng_get_option_length(length));
}

/*****************************************************************************
 * pcapng_write_shb()
 *****************************************************************************/
static bool pcapng_write_shb(struct pcapng_dumper* pd, const char* comment, const char* hardware, const char* os,
    ...
    if (user_application)
        shb_length += pcapng_get_option_length(strlen(user_application));
    ...
    if (comment)
        opt = pcapng_add_option(opt, PCAPNG_OPT_COMMENT, strlen(comment), comment);

    if (hardware)
        opt = pcapng_add_option(opt, PCAPNG_OPT_SHB_HARDWARE, strlen(hardware), hardware);

    if (os)
        opt = pcapng_add_option(opt, PCAPNG_OPT_SHB_OS, strlen(os), os);

    if (user_application)
        opt = pcapng_add_option(opt, PCAPNG_OPT_SHB_USERAPPL, strlen(user_application), user_application);
 ...
 * pcapng_write_idb()
 *****************************************************************************/
static bool pcapng_write_idb(struct pcapng_dumper* pd, const char* name, uint16_t snap_len, const char* description,
                             const uint8_t* mac, uint64_t speed, uint8_t ts_resolution, const char* hardware) {
    int rc;
    size_t idb_length;
    ...
    if (name)
        opt = pcapng_add_option(opt, PCAPNG_OPT_IDB_IF_NAME, strlen(name), name);

    if (description)
        opt = pcapng_add_option(opt, PCAPNG_OPT_IDB_IF_DESCRIPTION, strlen(description), description);

    if (mac)
        opt = pcapng_add_option(opt, PCAPNG_OPT_IDB_IF_MAC_ADDR, 6, mac);

    if (speed)
        opt = pcapng_add_option(opt, PCAPNG_OPT_IDB_IF_SPEED, sizeof(uint64_t), &speed);

    if (ts_resolution != 6 && ts_resolution != 0)
        opt = pcapng_add_option(opt, PCAPNG_OPT_IDB_IF_TSRESOL, sizeof(uint8_t), &ts_resolution);

    if (hardware)
        opt = pcapng_add_option(opt, PCAPNG_OPT_IDB_IF_HARDWARE, strlen(hardware), hardware);
    /* WARNING: If a new option is added, make sure the length calculation
     *          above is also updated!
     */
 ...
 * pcapng_write_epb()
 *****************************************************************************/
static bool pcapng_write_epb(struct pcapng_dumper* pd, uint32_t ifid, const uint8_t* pkt, uint32_t len, uint32_t caplen,
                             uint64_t timestamp, struct pcapng_epb_options_s* epb_options) {
    int i = 0;
    int rc;
    size_t pad_length;
    size_t com_length = 0;
    ...
    static struct xdp_verdict {
        uint8_t type;
        int64_t verdict;
    } __attribute__((__packed__)) verdict = {PCAPNG_EPB_VEDRICT_TYPE_EBPF_XDP, 0};
    ...
    if (epb_options->flags)
        opt = pcapng_add_option(opt, PCAPNG_OPT_EPB_FLAGS, sizeof(uint32_t), &epb_options->flags);

    if (epb_options->dropcount)
        opt = pcapng_add_option(opt, PCAPNG_OPT_EPB_DROPCOUNT, sizeof(uint64_t), &epb_options->dropcount);

    if (epb_options->packetid)
        opt = pcapng_add_option(opt, PCAPNG_OPT_EPB_PACKETID, sizeof(uint64_t), epb_options->packetid);

    if (epb_options->queue)
        opt = pcapng_add_option(opt, PCAPNG_OPT_EPB_QUEUE, sizeof(uint32_t), epb_options->queue);

    if (epb_options->xdp_verdict) {
        verdict.verdict = *epb_options->xdp_verdict;
        opt = pcapng_add_option(opt, PCAPNG_OPT_EPB_VERDICT, sizeof(verdict), &verdict);
    }
    /* WARNING: If a new option is added, make sure the length calculation
    ...
    if (epb_options->comment) {
        ...
        size_t opt_pad = roundup(com_length, sizeof(uint32_t)) - com_length;
        /* Add option header. */
        iov[i].iov_base = opt;
    ...
    iov[i++].iov_len = 8 + (epb_options->flags ? 8 : 0) + (epb_options->dropcount ? 12 : 0)
                       + (epb_options->packetid ? 12 : 0) + (epb_options->queue ? 8 : 0)
 ...
 * pcapng_dump_open()
 *****************************************************************************/
struct pcapng_dumper* pcapng_dump_open(const char* file, const char* comment, const char* hardware, const char* os,
                                       const char* user_application) {
    struct pcapng_dumper* pd = NULL;
 ...
 * pcapng_dump_close()
 *****************************************************************************/
void pcapng_dump_close(struct pcapng_dumper* pd) {
    if (pd == NULL)
        return;
 ...
 * pcapng_dump_flush()
 *****************************************************************************/
int pcapng_dump_flush(struct pcapng_dumper* pd) {
    if (pd != NULL)
 ...
 *****************************************************************************/
int pcapng_dump_add_interface(struct pcapng_dumper* pd, uint16_t snap_len, const char* name, const char* description,
                              const uint8_t* mac, uint64_t speed, uint8_t ts_resolution, const char* hardware) {
    if (!pcapng_write_idb(pd, name, snap_len, description, mac, speed, ts_resolution, hardware))
        return -1;
...
/*****************************************************************************
 * pcapng_dump_enhanced_pkt()
 *****************************************************************************/
bool pcapng_dump_enhanced_pkt(struct pcapng_dumper* pd, uint32_t ifid, const uint8_t* pkt, uint32_t len,
                              uint32_t caplen, uint64_t timestamp, struct pcapng_epb_options_s* options) {
    struct pcapng_epb_options_s default_options = {};

    return pcapng_write_epb(pd, ifid, pkt, len, caplen, timestamp, options ?: &default_options);
}
